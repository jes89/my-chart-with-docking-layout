...
- 메모이제이션 : 다큐먼트를 셀렉트할때 ID셀렉터가 가장 빠르지만, 도킹 레이아웃의 특성상 같은 레이아웃이 여러번 존재할 수 있어 ID 셀렉터를 사용할 수 없으며, 만약 사용한다해도 문서전체를 탐색하는 작업은 비효율 적이니 한번 탐색했던 엘리먼트는 캐쉬에 저장해 캐쉬에서 가져오는 형식으로 
